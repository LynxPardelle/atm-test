...
function getRdy2Config() {
  ...
  configDiv.innerHTML = `
  <h2 class="text-center my-3">Configuración del cajero</h2>
  <ul
  ...
  <button
    class="btn btn-warning d-block my-5 mx-auto"
    onclick="closeConfig()"
  >
...
function editConfigBtn() {
  ...
  if (isEdit === false) {
    ...
    if (editBTNDiv && editBTNDiv.innerHTML) {
      ...
      <button
        class="btn btn-success d-block my-3 mx-auto"
...
function updateAtmConfig() {
  for (let m in atm.config) {
    let colB = document.getElementById(`colB${m}`);
    if (colB && colB.value) {
      atm.config[m] = colB.value;
    }
  }
  isEdit = false;
  getRdy2Config();
